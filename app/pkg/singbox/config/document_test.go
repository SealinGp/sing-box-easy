...
func (f *fakeCoreAdapter) Version(context.Context) (core.CoreVersion, error) {
...
func TestValidateDocumentDelegatesUnknownFieldsToInstalledCore(t *testing.T) {
	...
	fake := &fakeCoreAdapter{version: core.CoreVersion{Major: 1, Minor: 14, Patch: 0, Raw: "1.14.0"}}
	manager.core = fake
	...
	if fake.validateCalls != 1 {
		t.Fatalf("core validation calls = %d, want 1", fake.validateCalls)
	}
	if !bytes.Equal(fake.validated, raw) {
		t.Fatalf("installed core received %s, want exact document %s", fake.validated, raw)
	}
...
func TestValidateRawConfigRejectsInvalidJSONBeforeCallingCore(t *testing.T) {
	...
	if fake.validateCalls != 0 {
		t.Fatalf("core validation calls = %d, want 0", fake.validateCalls)
	}
}
...
func TestSaveDocumentDoesNotReplaceActiveConfigWhenCoreRejectsCandidate(t *testing.T) {
	...
	manager.core = &fakeCoreAdapter{
		version:     core.CoreVersion{Major: 1, Minor: 13, Patch: 11, Raw: "1.13.11"},
