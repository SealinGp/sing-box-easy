...
to. Empty means the default. It must be https — see `app/pkg/subprobe` for why
...
- The quality prober is covered at three levels: pure aggregation (`subprobe/aggregate_test.go`), the store's bucketing and both retention bounds (`store_xorm_test.go`), and the runner against a fake prober (`runner_test.go`, including the not-imported vs not-running distinction). `clashapi/delay_test.go` pins the status-code mapping that decides "down" vs "untestable", and the tag escaping — real tags carry spaces, emoji flags and the ` | subID` suffix, none of which survive an unescaped path segment.
