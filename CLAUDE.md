...
# Run specific test
go test ./app/pkg/subscription/internal/feed/protocol -v -run TestTrojanParser
```
...
- `app/pkg/singbox/` - everything that serves the sing-box process, layered bottom-up (enforced by `app/architecture/layering_test.go`):
  ```
  singbox/core                    the binary: version (ONE parser), `sing-box check`, capability gates
  singbox/clashapi                the running process: client for its Clash-compatible API
  singbox/config                  config.Manager: raw document, sections, staging, validate, atomic rename
  singbox/config/history          DB-backed version history (implements config.VersionStore)
  singbox/config/sections         dns / route / inbound / experimental editors (raw JSON)
  singbox/install                 installs the binary (script / opkg), the Clash dashboard, the first config; setup state
  singbox/config/outbounds        outbound editor + subscription Reconciler
    ├── nodegroup                 selector/urltest building, member pruning, endpoint classification
    ├── nodetag                   fingerprinted node-tag minting and legacy-shape candidates
    └── rules                     Outbound Node Rules (Filters/Groups)
  singbox                         the process controller (package singbox, files at the root)
  ```
  `sections` and `outbounds` are siblings and never import each other: `subscription` imports `outbounds` for the Reconciler and must not drag the section editors in with it. `install` imports only `config` and `core` — host detection (`SystemType`) lives in `platform/sysinfo`, not in the controller. `nodegroup`/`nodetag` are leaves because both `outbounds` and `rules` need them — folding them into `outbounds` would be an import cycle. Every package's name matches its directory
- `app/pkg/singbox/config/` - sing-box config management with validation and rollback. `config.Service` is the whole-document + history surface the config handlers use
- `app/pkg/singbox/config/history/` - DB-backed config version history store (XORM) + retention cleaner (used by `config.Manager` snapshots and the rollback endpoints)
- `app/pkg/settings/` - Application settings persistence (XORM); served via `GET/PUT /settings`
- `app/pkg/singbox/` - sing-box service lifecycle control (start/stop/restart). A `Backend` interface (`backend.go`) abstracts the init system, detected in order: `systemd` (systemctl + journald), `procd` (OpenWrt `/etc/init.d/sing-box` + logread), `process` (direct spawn/signal). The `Controller` owns config validation and delegates lifecycle to the backend
...
- `app/pkg/platform/openwrtnet/` - OpenWrt host-network integration applied around the sing-box lifecycle. `DerivePlan` reads the config (as generic JSON — `Inbound.Options` is a registry-populated `any`) for a tun inbound and for the inbound named by a `hijack-dns` route rule; `Manager.Apply` then creates a `sing-box-easy` firewall zone for the tun device (`masq=0` on purpose: sing-box dials its own outbounds, so NAT would erase the client IP that route rules match on) and repoints dnsmasq at sing-box's DNS port, recording the previous upstreams. It also exempts the names a `hosts` DNS server maps to a private address from dnsmasq's rebind protection (`rebind_domain`) — once dnsmasq forwards to sing-box those answers arrive from an "upstream" and `stop-dns-rebind` strips them, so the client sees NOERROR with zero records for a name sing-box resolved correctly. Every dhcp edit is staged and committed together so dnsmasq restarts once. `Manager.Revert` restores the upstreams and removes only the exemptions it added. State lives in `/etc/sing-box/.openwrt-net-state.json` so it survives a panel restart. Every operation is a no-op off OpenWrt. Wired into `singbox.Controller`: apply **before** `backend.Start` (the tun device appears with sing-box, and until its zone exists fw4 resets everything coming back off it), revert **after** `backend.Stop`
- `app/pkg/diagnostics/ruleset/` - Evaluates `route.rule_set` **without a running sing-box**. Remote sets are read from the same bbolt cache file sing-box populates (`experimental.cache_file`), local ones from their path; both are decoded with sing-box's own `common/srs`. bbolt gives writers an exclusive flock, so a read-only open blocks while sing-box runs — the file is then copied and the copy read. Matching is reimplemented in `matcher.go` rather than delegated to `route/rule`, which would drag the whole sing-box runtime (sing-tun, gvisor, tfo-go, quic-go) into a panel that otherwise depends on `option` + `constant` alone; each rule carries its upstream line reference. Verdicts are **tri-state** — "could not read geoip-cn" and "not in geoip-cn" route differently, and a two-state matcher has to guess
- `app/pkg/diagnostics/route/` - The route simulator behind `POST /route/probe`. Walks `route.rules` the way `route/route.go` does and reports where a destination *would* go, before any traffic exists
- `app/pkg/singbox/clashapi/` - The panel's client for sing-box's Clash-compatible API: controller URL resolution (`external_controller` is a LISTEN address — `:9090`/`0.0.0.0:9090` are rewritten to loopback), bearer, status classification, and typed `Connections()`/`Rules()`. The DNS probe's client wraps it. **Plain HTTP only**: every sing-box streaming endpoint also answers a plain GET (`/connections` = one snapshot, `/traffic` = chunked JSON lines), so there is no WebSocket dependency. Two payload facts the package documents because both differ from what a Clash dashboard would lead you to expect: `chains` is **reversed** (index 0 is the leaf actually dialled, the LAST element is the outbound the rule named), and `rule` is `<rule.String()> => <action.String()>` — or the literal `final` — with **no rule index anywhere**. Verified identical across sing-box 1.12.12, 1.13.12 and the 1.14 beta
- `app/pkg/traffic/` - Turns `/connections` snapshots into the frames behind `GET /traffic/flow/stream` (the Overview diagram's Live overlay). Three derivations the raw payload lacks: **rates** (`Differ` diffs cumulative counters over wall-clock seconds, not per tick, so a late poll is not a burst; a counter that went DOWN — sing-box restarted and reused an id — clamps to 0 rather than drawing a ribbon backwards); **rule index** (`RuleIndex` maps the rule string back to a config position using sing-box's own `/rules`, which returns `router.Rules()` in config order with `payload = rule.String()` and `proxy = action.String()` — the string a connection carries is exactly `payload + " => " + proxy`, so no matcher `String()` is reimplemented; on duplicates the first index wins, since sing-box stops at the first match); and **the exit** (last chain element) with the leaf as `via`. `Aggregate` folds connections onto edges keyed the way the frontend layout is keyed (inbound tag, rule index, exit tag), applies the `source_ip`/`host` filter, and keeps the top three hosts per rule. It also reports `sources` — every client address in the snapshot, with its connection count — collected **before** the filter and ordered by address: it feeds the UI's device picker, and a list built from the filtered connections would collapse to the one address just chosen, leaving no way back to another device. `Run` polls at 1 Hz and re-reads `/rules` when an unmatched string appears, but only after a cooldown: a connection opened before a reload keeps its old rule string for its whole life, and no number of re-reads maps it
- `app/pkg/subscription/internal/probe/` - Subscription **quality metrics**: availability and average latency per subscription, sampled on a schedule. Answers the question the quota figures cannot — is this feed worth renewing — which only time can answer, so it has to be sampled and kept. The measurement is **delegated to sing-box**: its Clash API `GET /proxies/{tag}/delay` (`clashapi/delay.go`) dials the target through that exact outbound with its real TLS/transport settings, so all six protocols work for free and the number describes a connection someone actually makes. The cost is a stated precondition — probing needs sing-box running with `clash_api`, the same requirement the Overview live view already has. **The `url` MUST be https**: sing-box's delay handler does `if strings.HasPrefix(url, "http://") { url = "" }` and silently substitutes its own default, so an accepted `http://` target would report latency for an endpoint the operator never chose, with nothing on screen disagreeing — `subscription.NormalizeProbeURL` refuses it at the boundary and `EffectiveProbeURL` re-validates at read time (a row written before the rule existed, or by another API client, must not reach the prober). Node results are **tri-state**, not two: a node absent from the RUNNING sing-box is *untestable*, not down, and is excluded from the availability denominator — counting it would blame the provider for an unapplied config. The two empty outcomes stay separate errors (`ErrNoOwnedNodes` vs `ErrNoNodesTestable`) because they look identical from outside and have opposite fixes — "refresh the subscription" against "restart to apply the config"; collapsing them was a real bug, found by running the prober against a live sing-box whose config had been re-tagged. Nothing is recorded when sing-box is unreachable or the secret is wrong: the panel being unable to ask is not evidence of an outage, and a 0% point written there stays in the chart forever. Storage is **one aggregate row per subscription per run** (`repo.ProbeSample`, ~80 B) — per-node history would be ~60x the rows to answer a question only ever asked about right now, so the latest run's per-node detail lives in memory instead. Two retention bounds apply, **trimmed on the write path** rather than by a cron (a misconfigured interval would otherwise fill a router's overlay between nightly sweeps): an age window and a per-subscription point cap, whichever bites first. Measured on the production config: a 37-node subscription sweeps in **5.2 s**, and 103 all-dead nodes take **65 s** at the fixed 8-way concurrency — comfortably inside the 10-minute default. Latency is averaged over the REACHABLE nodes only; folding a timeout in as 5000 ms would make a partly-dead feed read as a slow one, which is a different problem with a different fix. `Runner.Stop` waits for **any** run in flight, not just the scheduled sweep — the per-row "probe now" button runs on its own HTTP goroutine outside the scheduler loop, so waiting only for the loop returned while a manual probe was still writing; it also cancels a shutdown context that both paths derive from, or a Stop arriving just after a sweep began would block for the sweep's 15-minute ceiling. Both are pinned by `lifecycle_test.go`, which fails against the previous implementation. One run at a time is a deliberate shared gate: a long manual probe makes that interval's sweep skip, which is preferable to two sweeps doubling the load on a device that is also routing traffic
- `app/pkg/platform/process/` - Process discovery and signaling helpers (pgrep/SIGTERM/SIGHUP)
...
**Protocol Parsers** (`app/pkg/subscription/internal/feed/protocol/`):
...
The `singbox.Controller` manages sing-box lifecycle:
...
endpoint (`nodetag.FingerprintEndpointKey`) and grepping for it. md5 is an
...
- `legacyTagValue` (singbox/config/outbounds/rules/tagparts.go) converts a saved matcher/exclude
...
`POST /outbounds` — via `nodetag.OutboundTagCandidates`, whose first element is
...
A subscription carries `probe_enabled` (default **true**, including for rows that
predate the feature — an opt-in metric leaves the quality chart empty on every
existing install, which reads as the feature being broken rather than as it
being off) and `probe_url`.

The URL is **per subscription** because a provider whose nodes cannot reach
Google reads as 0% available against `gstatic` while working perfectly; the
operator has to be able to point the test somewhere the feed can actually get
to. Empty means the default. It must be https — see `app/pkg/subscription/internal/probe/` for why
that is a correctness rule and not a preference.

Note for anyone adding a field here: `ManagerXORM.Update` writes an **explicit
column list**, so a new column must be added to `cols` or it is silently never
persisted. And because Go decodes an absent JSON bool as `false`, the add/update
handlers read `probe_enabled` out of the raw body (`probeEnabledFromBody`) to
tell "not sent" from "set to false" — without that, any client that predates the
field would turn probing off on every save.
...
- **Subscription quality** (`SubscriptionQualityCell` + `SubscriptionQualityDialog` + `ProbeTrendChart`, geometry in `utils/probeChart.ts`): availability and latency over time, per subscription. **Two stacked panels sharing one x-axis, not one chart with two y-axes** — the quantities are unrelated and in unrelated units, and a dual-axis chart makes the reader work out which line belongs to which scale before either number means anything. No chart library: it is two line panels, and a dependency to draw them would outweigh the screen. Availability is drawn on an **absolute 0–100% axis**, never scaled to the data, so a feed that never dropped below 90% is not drawn hugging the floor. A run where nothing answered is **excluded from the latency line** (its `avg_ms` is 0, and plotting that renders a total outage as the fastest moment on the chart) but **included in availability**, which is the whole point of it. The line **breaks where sampling stopped** — the panel records nothing while sing-box is down, and a line drawn straight across that hole asserts an availability never measured, precisely over the window where something was wrong; the break is derived from the median spacing so it works at any server-chosen bucket. The dialog's headline is the **latest single run**, taken from the in-memory node snapshot rather than the last chart point: on a bucketed range that point sums several runs, and it read "331/333 nodes" for a subscription with 37. The node table sorts **worst-first**, since the reason to open it is to find what broke. The availability figure is accompanied by `SegmentedProgress` (`components/SegmentedProgress.vue`, Ant Design's `<Progress steps>`): discrete blocks, because the quantity IS counted — a continuous bar for "36 of 37 nodes" draws something never measured continuously. It is used in **composition** mode (`percent=100`, a per-block colour array), so the blocks show how the node population splits rather than filling to a percentage. `utils/qualitySteps.ts` builds that array and its rounding rule is the point: the green count **floors**, and both minorities are clamped into view — any failure keeps at least one block red, any success at least one green. Arithmetic rounding would paint 36/37 (97.3%) as five green blocks, drawing a feed with a dead node as flawless, and a health gauge that can round a failure away is worse than none because it is trusted. The cost is that one dead node in 37 paints a fifth of the bar; that is accepted because the exact figure is always rendered beside it, so the blocks are the glance and the text is the truth. The Settings card states the disk cost as a **measured** row count and size, not an estimate — the operators most affected are on routers whose root overlay is a few tens of MB
- **Overview traffic-flow card** (`RouteTopologyCard` + `RouteFlowDiagram`, model in `utils/routeTopology.ts`, geometry in `utils/flowLayout.ts`): draws `inbounds → route.rules → outbounds` from one `GET /config` read. The Route page lists rules as rows, which is right for editing one and wrong for seeing what the config does — two facts are invisible in a list. First, **several rules routinely share one outbound**: the production config reaches `🤖 AI` from three unrelated rules, so exits are keyed by tag and the ribbons converge on a single node, fanned across its edge. The convergence IS the OR; a node drawn per rule would claim three outbounds where sing-box has one. Second, the middle column is an **ordered ladder** that stops at the first match, so it is drawn as one.
  ...
  Termination is copied from `app/pkg/diagnostics/route/rule_meta.go`, not reasoned about: only `route`, `reject` and `hijack-dns` stop the walk, and the `direct` ACTION does **not** — sing-box's `route.go` has no case for it. Mirroring that upstream quirk is deliberate; a picture that silently corrects the engine is a picture of a config that is not running. The three fall-through cases (`route.final` / first outbound / synthesised direct) stay separate for the same reason they do in `diagnostics/route` — reporting "final" for all three sends someone hunting for a key that is not in their config.
  ...
  **Live toggle.** The same drawing lit by real connections (`useTrafficFlow` → `/traffic/flow/stream`, re-keyed by `utils/flowOverlay.ts`). The picture never changes shape between the two modes — the overlay only widens, lights and animates what is already there — so expected and actual are compared on ONE diagram. **"Busy only" is the one deliberate exception**, and it is opt-in and live-only for exactly that reason: it folds the rules carrying nothing into collapsed bands (`layoutRouteFlow`'s `hiddenRuleIndices`), because a floor quiets a 28-rung ladder without SHORTENING it and 22 dim rungs still have to be scrolled past. A band sits **where its rules sat** and merges only CONTIGUOUS runs — the ladder stops at the first match, so a band that moved, or one pooling #3 with #19, would misstate the evaluation order the diagram exists to show — and clicking one puts its rules back, since a fold that cannot be undone hides the rule someone came for. What counts as carrying traffic is `RATE_FLOOR` (1 KB/s, down+up), not "appears in the frame": one DNS lookup or keepalive per rule is enough to light the whole ladder, at which point lighting distinguishes nothing. Suppressed rows keep their rates, connection counts and hosts, and the live strip says how many there are — a diagram that went quiet has to explain itself or it reads as a broken stream. Ranking for the pulse happens among the lit flows only, so an idle router animates nothing rather than ranking eight nothings. Only the **top 8** rule ribbons by download rate animate (`TOP_N`); everything else above the floor is lit but still, and idle elements step back to 35% rather than vanish. Dash velocity is a **log** function of bytes/s (`dashDurationFor`): 10 KB/s and 10 MB/s must both read as motion, and on a linear scale everything below the fastest edge looks stopped. **Colour is heat** (`heatFor` → `utils/flowHeat.ts`): a function of ABSOLUTE download rate, one tier per decade above the floor (blue, blue, amber, orange), so an extreme is a different colour from its neighbours rather than a slightly darker one, and 1 MB/s looks the same on a quiet router and a busy one — relative-to-frame colouring would paint an idle router's 50 KB/s as blazing. The busiest **three** rows (`RANK_N`) carry a numbered mark on the wire where their ribbon leaves — placed from the ROW, since a pass-through rule has no ribbon and a rank assigned but never drawn reads as "1, 2, 4" — and the mark deliberately never animates: two close flows swap places every second. The **comet** (faint tail, bright head) travels linearly because a rate is constant along its path. An arrival ring that lit the outbound as each comet landed was tried and removed — on a busy exit it is a blink every few hundred milliseconds — so the outbound's heat-coloured outline is the only "receiving" signal. Lighting a ribbon takes 200 ms and quieting it 300 ms (`.fade-live.is-lit`), both on the shell's `cubic-bezier(.32,.72,0,1)`. Live is **on by default while sing-box is running** — the card's question is "is my config doing what I meant", and the overlay is the half that answers it, so a click-first default made the landing view the one with less in it; a stop turns it off and a start turns it back on, which is what makes the view survive the restart the panel triggers on every save. The source filter is a **picker over the frame's `sources`**, not a text box: the panel already knows every client address holding connections, and a typed address with a typo looks exactly like a device with no traffic. The strip carries **no tint**: a full-width tinted bar above content reads as a HEADER, when the row is the opposite of a label — values changing every second plus the controls that shape them — and liveness is already stated by the toggle's own state and by the diagram lighting up. Its two halves are told apart by WEIGHT instead: readouts get the app's uppercase micro-label idiom and a `text-sm` number, with **down** taking the accent because ribbon width, pulse speed and top-N ranking are all functions of it; counts and caveats stay secondary; and the filters keep their own borders, which only read as borders against the card rather than flattening into a tint. The symbol legend under the diagram is **collapsible and persisted** (`sbe-routeflow-legend`) — it is reference material a new install needs two or three times and then never again, so it shelves permanently on one click rather than re-opening every visit. The COLUMN strip stays out of that collapse: it carries the claim that the ladder is ordered and stops at the first match, which is the diagram's thesis rather than a symbol key, and it is the only place that claim survives now that `routeFlow.desc` is gone. The toggle is **disabled unless the service store reports `running`** — the data is served by sing-box itself, so a clickable button that always fails reads as a panel bug; a stop while Live is on turns it off. While on, a dropped stream reconnects after 3 s, because the commonest drop is the restart the panel itself triggers on every config save — precisely when the operator wants to watch. Rates are formatted through `formatRate` (rounded first): they are derived values with floating-point noise, and `formatBytes` — written for integer counts — passes `391.55309828159835 B` straight through below 1 KB. **Full-window mode** moves the card's body into a viewport-filling overlay with one `<Teleport :disabled>` — it MOVES the same subtree rather than rendering a second copy, so the live stream, filter text and hover state survive the switch; a dialog would mount fresh content and reconnect for nothing. In-window rather than the Fullscreen API on purpose (the operator is comparing against a terminal or the Clash dashboard). The diagram takes a `fit` prop there: fill the HEIGHT and keep the ratio, so the whole ladder is on screen at once — the point of the mode — letterboxing on a wide monitor rather than scrolling. **Zoom** (`composables/useDiagramZoom.ts`) exists because both default sizings SHRINK a fixed 1290px canvas — `max-w-full` docked, `fit` full-window — until an 11px rule label renders at six: readable as a shape, unreadable as a sentence, and the card still looks like it fits. It renders the same `viewBox` at a larger pixel size and lets the existing wrapper scroll, rather than transforming the viewBox: panning is then the browser's (two-finger, shift+wheel, scrollbars, arrow keys), already correct at the edges where a hand-rolled transform clips `<title>` hit areas, and SVG scales without resampling so labels get genuinely sharper. `scale === null` means "leave the default alone", so nothing changes for an operator who never touches the control; steps come from a fixed ladder so 100% is always reachable and repeated clicks are reversible, and the step from `null` is taken from the MEASURED factor (`viewBox` width vs rendered width) — stepping from a guess of 1 would jump a shrunk diagram straight to 125%. **Ctrl/⌘+wheel** (which is also how a trackpad pinch arrives) needs `addEventListener(..., { passive: false })`, not an `@wheel` binding: without the `preventDefault` a passive listener cannot cancel, and the browser zooms the page instead. **Ctrl/⌘+drag** pans by moving the same `scrollLeft`/`scrollTop`, so the drag, the scrollbars, the wheel and the keyboard address one position and cannot disagree. It is grab-the-canvas (pointer right → picture right → scroll LEFT), and offsets are recomputed from the drag ORIGIN rather than accumulated per delta: accumulating drifts against the DOM's own clamping, so dragging past an edge and back leaves a dead zone before the picture moves again. `pointercancel` ends a pan as well as `pointerup` (a system gesture delivers a cancel and no up, which would leave the diagram stuck to a buttonless cursor), the modifier is deliberately NOT re-checked mid-drag (it arms the gesture; re-checking abandons the pan when a finger slips), and `contextmenu` is suppressed while a modifier is held because on macOS ctrl+click IS a right-click. The control itself **floats on the diagram**, not in the card header: it is adjacent to what it scales, and being positioned against the wrapper rather than inside the scrolling child it stays reachable at any pan position
...
1. **`cmd/gen-option-schema`** reflects over the `option.*` structs that
   `app/pkg/singbox/config/registry.go` constructs and emits one inventory per domain —
   ...
   `go generate ./app/pkg/singbox/config/`. Adding the next domain (outbounds,
...
- **Some outbounds are not owned by the form.** `outbounds/rules` ->
  ...
  `managed_tags` (from `nodegroup.ManagedOutboundTags`) and the dialog warns.
...
1. Create `app/pkg/subscription/internal/feed/protocol/newprotocol.go`
...
Every write goes through `config.Manager` (`app/pkg/singbox/config`), which
stages the file, runs `sing-box check`, snapshots the old version into history
and renames atomically. Pick the entry point by how much of the document the
edit needs to understand:

- **One section, as raw JSON** — `UpdateConfigSection(ctx, "dns", fn)`, where
  `fn` receives and returns that section's `json.RawMessage`. This is the
  default: it never decodes through the pinned `option` structs, so fields a
  newer sing-box added survive the edit. The section editors in
  `singbox/config/sections` use nothing else.
- **Outbounds, typed** — `UpdateOutboundsConfig(ctx, func(*config.SingBoxConfig) error)`.
  Only the outbound list is decoded; every other section passes through raw,
  and unknown outbound fields are merged back by tag. Used by
  `singbox/config/outbounds`, because renames have to be carried into
  selector/urltest members.
- **The whole document** — `config.Service.Save` (raw bytes in, validated,
  promoted). Handlers never call `Manager` directly; `app/architecture` fails
  the build if one does.

There is no typed whole-config write any more: it re-encoded every section
through the pinned schema and silently dropped what a newer core added.
...
the config CRUD in `app/pkg/singbox/config/sections/` is raw JSON throughout and
...
- `/subscription-probes/{status,settings}`, `/subscription-probes/:id/{history,nodes,run}` — subscription quality metrics (see `app/pkg/subscription/internal/probe/`). Its own prefix rather than `/subscriptions/...` because that path already owns a `:id` wildcard at the next segment, which a static sibling collides with in the Hertz router — the same reason `/settings/subscription-info-keywords` lives where it does. `history` takes a **named** `range` (1h/6h/24h/7d/30d) rather than free-form from/to, because the server picks the bucket alongside the window: at the 1-minute interval floor a 30-day window is ~43k rows, and a client choosing one without the other is how that arrives unbucketed. Bucket arithmetic is deliberately not a mean of means — availability is `sum(reachable)/sum(total)` so a 100-node run outweighs a 4-node one, latency is weighted by node count, and an all-dead run contributes no latency at all rather than dragging the average toward zero and drawing a dead hour as the fastest one
...
- `/subscription-probes/settings` (PUT) — probe interval, per-node timeout, retention days and point cap. Separate from `/settings` because the four are validated as a group and reported back with the resulting storage figure, which the generic endpoint has no shape for. Validated as a group **literally**: `settings.Validate*` are split from `Set*` because each setter commits its own transaction, so validating by attempting the write persisted every field ahead of the one that failed — the client got an error and reasonably assumed nothing had changed. `POST /subscription-probes/:id/run` is synchronous and its client call carries an **explicit 11-minute timeout**: the shared axios default is 30s, but a down node costs the full per-node timeout, so a large dead feed (measured: 103 nodes, 65s) would be aborted by the browser and reported as failed while the server completed it and stored the point. The timeout ceiling is 8s, set by the Clash API client's own 10s request timeout: a longer one would be cut off by the transport before sing-box could answer its 504, turning every slow node into an unexplained error
...
- `/system/info` — host details for the Settings "About" card: arch, CPU cores, kernel, distribution, hostname, service backend, the sing-box + sing-box-easy versions, and `disks` (free space on the filesystems the panel writes to). Collected by `app/pkg/platform/sysinfo/`, which degrades every field to a zero value rather than failing. `disks` exists because a full filesystem is otherwise invisible: SQLite cannot create its rollback journal and reports `SQLITE_CANTOPEN` ("unable to open database file (14)"), which names the database and never the disk — reads keep working, so it reads as a permissions bug on the `.db` file. `sysinfo.CollectDisks` statfs's each path and resolves the mount point/device from `/proc/self/mounts` (longest-prefix match, octal-unescaped), collapsing paths that share a filesystem. `database.AnnotateWriteError` wraps the same diagnosis onto write failures, but only after statfs confirms it, so a genuine permission error is never mislabelled. This matters most on OpenWrt, where the root overlay is often a small loop image while terabytes sit mounted elsewhere
...
- The quality prober is covered at three levels: pure aggregation (`subscription/internal/probe/aggregate_test.go`), the store's bucketing and both retention bounds (`store_xorm_test.go`), and the runner against a fake prober (`runner_test.go`, including the not-imported vs not-running distinction). `clashapi/delay_test.go` pins the status-code mapping that decides "down" vs "untestable", and the tag escaping — real tags carry spaces, emoji flags and the ` | subID` suffix, none of which survive an unescaped path segment.
...
- `app/pkg/singbox/config/types.go` - sing-box config struct definitions
