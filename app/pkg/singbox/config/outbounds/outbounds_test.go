...
func (c *countingCore) Version(context.Context) (core.CoreVersion, error) {
	return core.CoreVersion{Major: 1, Minor: 12, Patch: 12, Raw: "1.12.12"}, nil
}
...
func newManager(t *testing.T, raw string) (*config.Manager, *countingCore, string) {
	...
	checks := &countingCore{}
	return config.NewManagerWithCore(path, checks), checks, path
}
...
func TestAddOutboundsWritesNothingWhenEveryNodeExists(t *testing.T) {
	...
	m, checks, path := newManager(t, `{"outbounds":[
	...
	if checks.validations != 0 {
		t.Errorf("sing-box check ran %d times for a no-op add", checks.validations)
	}
...
func TestReconcilerApplyWithNoChangesDoesNotWrite(t *testing.T) {
	m, checks, _ := newManager(t, `{"outbounds":[{"type":"direct","tag":"direct"}]}`)
	err := NewReconciler(m, nil).Apply(context.Background(), "sub_test", func(*config.SingBoxConfig) Changes {
		return Changes{}
	})
	if err != nil {
	...
	if checks.validations != 0 {
		t.Errorf("sing-box check ran %d times for an empty change set", checks.validations)
	}
}
