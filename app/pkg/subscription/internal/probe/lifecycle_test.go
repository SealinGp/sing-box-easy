...
func TestDeletionDrainsAndBlocksStaleProbeTargets(t *testing.T) {
	store := newStore(t)
	prober := newBlockingProber()
	runner := NewRunner(store, lifecycleEnv(prober))
	done := make(chan struct{})
	go func() { defer close(done); _, _ = runner.RunSubscription(context.Background(), "sub_a") }()
	select {
	case <-prober.started:
	case <-time.After(5 * time.Second):
		t.Fatal("probe did not start")
	}
	runner.BeginDelete("sub_a")
	// Once BeginDelete returns no publisher can recreate history or snapshots.
	if err := store.DeleteSubscription("sub_a"); err != nil {
		t.Fatal(err)
	}
	<-done
	if runner.LatestNodes("sub_a") != nil {
		t.Fatal("snapshot survived deletion")
	}
	if _, err := runner.RunSubscription(context.Background(), "sub_a"); err == nil {
		t.Fatal("stale target was admitted")
	}
	if n, err := store.CountAll(); err != nil || n != 0 {
		t.Fatalf("late history write: %d %v", n, err)
	}
}
