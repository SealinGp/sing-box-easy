...
- `app/pkg/singbox/` - everything that serves the sing-box process, layered bottom-up (enforced by `app/architecture/layering_test.go`):
  ...
  ```
  `sections` and `outbounds` are siblings and never import each other: `subscription` imports `outbounds` for the Reconciler and must not drag the section editors in with it. `nodegroup`/`nodetag` are leaves because both `outbounds` and `rules` need them — folding them into `outbounds` would be an import cycle. Every package's name matches its directory
...
- `app/pkg/installation/state/` - Initialization state management (database-backed)
