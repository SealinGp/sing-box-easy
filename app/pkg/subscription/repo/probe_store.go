...
type Point = model.ProbePoint
type Sample = model.Sample

// ProbeStore persists probe samples.
type ProbeStore struct {
	e *xorm.Engine
}

// NewProbeStore creates a new XORM-backed probe sample store.
func NewProbeStore(e *xorm.Engine) *ProbeStore { return &ProbeStore{e: e} }

// Init ensures the subscription_probe_samples table exists.
func (s *ProbeStore) Init() error {
	if err := s.e.Sync2(new(ProbeSample)); err != nil {
...
func (s *ProbeStore) Insert(subID string, at time.Time, sample Sample) error {
	row := &ProbeSample{
		SubID:     subID,
		At:        at.Unix(),
		Total:     sample.Total,
...
func (s *ProbeStore) Series(subID string, from, to time.Time, bucket time.Duration) ([]Point, error) {
	if bucket <= 0 {
...
func (s *ProbeStore) rawSeries(subID string, from, to time.Time) ([]Point, error) {
	var rows []ProbeSample
...
func (s *ProbeStore) Latest() (map[string]Point, error) {
...
func (s *ProbeStore) Trim(subID string, maxAge time.Duration, keepN int) (int64, error) {
	var deleted int64

	if maxAge > 0 {
		cutoff := time.Now().Add(-maxAge).Unix()
		n, err := s.e.Where("sub_id = ? AND at < ?", subID, cutoff).Delete(new(ProbeSample))
		if err != nil {
	...
	if keepN > 0 {
		...
		if len(boundary) > 0 {
			n, err := s.e.Where("sub_id = ? AND at < ?", subID, boundary[0].At).Delete(new(ProbeSample))
			if err != nil {
...
func (s *ProbeStore) DeleteSubscription(subID string) error {
	if _, err := s.e.Where("sub_id = ?", subID).Delete(new(ProbeSample)); err != nil {
...
func (s *ProbeStore) CountAll() (int64, error) {
	n, err := s.e.Count(new(ProbeSample))
	if err != nil {
