...
- `app/pkg/integrations/clashapi/` - The panel's client for sing-box's Clash-compatible API: controller URL resolution (`external_controller` is a LISTEN address — `:9090`/`0.0.0.0:9090` are rewritten to loopback), bearer, status classification, and typed `Connections()`/`Rules()`. The DNS probe's client wraps it. **Plain HTTP only**: every sing-box streaming endpoint also answers a plain GET (`/connections` = one snapshot, `/traffic` = chunked JSON lines), so there is no WebSocket dependency. Two payload facts the package documents because both differ from what a Clash dashboard would lead you to expect: `chains` is **reversed** (index 0 is the leaf actually dialled, the LAST element is the outbound the rule named), and `rule` is `<rule.String()> => <action.String()>` — or the literal `final` — with **no rule index anywhere**. Verified identical across sing-box 1.12.12, 1.13.12 and the 1.14 beta
