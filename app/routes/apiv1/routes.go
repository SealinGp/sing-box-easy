...
// RegisterRoutes registers the API on every prefix in apiPrefixes.
//
// Declared once and mirrored, rather than written per prefix. A previous,
// partial migration hand-copied five endpoints onto /api/v1 and left the other
// 143 behind, so the two prefixes disagreed about what the API contained —
// exactly the drift a single declaration makes impossible.
func RegisterRoutes(h *server.Hertz, handler *Handler) {
	...
	auth := v1.with(AuthMiddleware(handler.userManager, handler.authEnabled))
	...
	admin := v1.with(AuthMiddleware(handler.userManager, handler.authEnabled), RequireAdmin())
