...
export default {
  ...
  rules: {
    ...
    actionTypes: {
      ...
      evaluate: 'Evaluate - 查询服务器并保存响应',
      respond: 'Respond - 返回已保存的查询响应',
    },
    rejectMethods: {
    ...
    flow: {
      then: {
        ...
        evaluate: '通过 {server} 查询并将结果保存为 {tag}',
        evaluateIncomplete: '通过 DNS 服务器查询并保存结果',
        respond: '返回已保存的响应 {tag}',
      },
    },
    form: {
      ...
      fields: {
        ...
        tag: '响应标签',
        timeout: '查询超时',
        match_response: '要匹配的响应标签',
        response_rcode: '要求的响应码',
        race: '竞速响应',
      },
      errors: {
        ...
        evaluateServerRequired: 'Evaluate 动作必须指定 DNS 服务器。',
        evaluateTagRequired: 'Evaluate 动作必须指定响应标签。',
        matchResponseRequired: 'Respond 动作必须指定 Evaluate 产生的响应标签。',
      },
      ...
      queryType: '查询类型',
      queryTypePlaceholder: '任意类型（例如 A、AAAA、HTTPS）',
      queryTypeHelp: '仅匹配这些记录类型的查询；留空则匹配所有类型。与其他条件为“与”关系。',
    },
    serverSelect: '选择服务器',
